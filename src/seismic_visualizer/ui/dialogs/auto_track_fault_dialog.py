"""AutoTrackFaultDialog — UI for ML fault auto-tracking."""
...
class AutoTrackFaultDialog(QDialog):
    """Dialog for running ML auto-tracking on a fault entity.
    ...
    Faults are tracked along inline or crossline — on vertical sections the
    fault plane appears as an inclined line, which the model can latch onto.
    """
    ...
    def __init__(
        ...
        self._worker: FaultTrackingWorker | None = None
        self._entity_ids: list[int] = []

        self.setWindowTitle(tr("autotrack_f.title"))
    ...
    def _build_ui(self, current_entity_id: int | None, current_axis: Axis) -> None:
        ...
        entities = self._project.fault_registry.all()
        ...
        form.addRow(tr("autotrack_f.entity"), self._entity_combo)
        ...
        if not entities:
            ...
            self._status_label.setText(tr("autotrack_f.no_entities"))
    ...
    def _collect_seeds(self) -> list[Point3D]:
        ...
        if entity_id is None:
            return []
        data = self._project.faults.data
    ...
    def _on_run(self) -> None:
        ...
        if not seeds:
            QMessageBox.warning(
                self,
                tr("autotrack_f.no_seeds_title"),
                tr("autotrack_f.no_seeds_msg"),
            )
            return
        ...
        try:
            ...
            tracker = OnnxFaultTracker(self._model_path)
        ...
        self._worker = FaultTrackingWorker(
